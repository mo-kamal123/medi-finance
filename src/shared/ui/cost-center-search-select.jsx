...
import {
  searchCostCenters,
  getCostById,
} from '../../features/accounting/tree/cost-tree/api/cost-tree';
...
const CostCenterSearchSelect = ({
  ...
  const selectedInResults = useMemo(
    () =>
      searchResults.find(
        (r) => String(r.costCenterID ?? r.id) === String(value)
      ),
    [searchResults, value]
  );
  ...
  const { data: selectedCenter } = useQuery({
    ...
    queryFn: async () => {
      const res = await getCostById(value);
      return res?.data ?? res;
    },
  ...
  const displayLabel = displayCenter
    ? `${displayCenter.costCenterCode || displayCenter.code || ''} - ${displayCenter.nameAr || displayCenter.nameEn || displayCenter.name || ''}`
    : value || '';
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-600">
                        {center.nameAr || center.nameEn || center.name || ''}
                      </div>
                    </div>
                  );
                })
